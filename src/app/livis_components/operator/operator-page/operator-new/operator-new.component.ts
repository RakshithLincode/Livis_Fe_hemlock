...
export class OperatorNewComponent implements OnInit {
  ...
  loadflagForm() {
    this.flagForm = this._fb.group({
      message: ['', [Validators.required]],
    });
  }
  cancelFlagModal(){
    this.flagForm.reset()
    $("#flag_btn").removeClass('invisible')
  }
  FlagForm(){
    $("#flag-process-modal").modal('show')
    $("#flag_btn").addClass('invisible')
  }
  submitFlag(model) {
    ...
    console.log(model)
    this.operatorService
      .sendFlagMessage({message : model.value.message, isFlagged : true, inspection_id : this.running_inspection_id})
      .subscribe((data) => {
  ...
  cameraLayoutLoad() {
    ...
    for (var i = 0; i < count_div; i++) {
      if (camera_list[i]) {
        ...
        <img style="width:100%;height:100%" src="${camera_list[i]
          }" id="live_feed_${i + 1}" alt="feed not available" >
          </div>`;
      } else {
        html += `<div style="position:relative !important ;padding-right:5px;padding-left:5px; height:${count_div <= 2 ? '85vh' : '42vh'
  ...
  processEndForm(popup = false) {
    if(popup){
      ...
      const end_info = { inspection_id: this.running_inspection_id };
          ...
          swal({
            title: '',
            html: "<h3 class='text-danger'>Process Ended due to Interuption.</h3>",
  ...
  getMetrix() {
    ...
    this.marutiService.getMetrix(this.running_inspection_id).subscribe((data: any) => {
      ...
      //////console.log(this.selected_part_item)
        if (this.plctrigger!= data.plc_trigger &&  data.plc_trigger == 'True') {
          this.Apicalldone = true;
          this.buttonText = "Please wait ..."
        }else if (this.plctrigger!= data.plc_trigger &&  data.plc_trigger == 'False'){
        ...
        if(this.retryClicked &&  data.retry_inspection == 'True'){
          this.Apicalldone = true;
          this.buttonText = "Please wait ..."
        }else if( data.retry_inspection == 'False' && data.retry == 'True'){
          this.Apicalldone = false;
          this.buttonText = "Inspect"
        }
      this.inspection_status = data.status;
      this.inferenceFrame = data.inference_frames
      // console.log(this.retry, this.retry!= data.retry, data.retry, typeof data.retry, typeof this.retry, data.retry == 'True')
      if(this.retry!= data.retry && data.retry == 'True'){
        console.log(this.retry, data.retry)
        // alert("yes")
        swal({
          title: '',
          text: 'Label Angle mismatched',
          type: 'warning',
          ...
          if (1) {
            if (result.value) {
              swal({
                title: '',
                text: 'Do you want to redo inspection ?',
                type: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Retry',
                cancelButtonText: 'Cancel',
                ...
                if (1) {
                  if (result.value) {
              this.marutiService.retryInspection().subscribe((data: any) => {
                this.retryClicked = true
                this.alertService.alertMessage(
                  'Retry Inspection Started'
                  ,"success","check"
                );
              })
