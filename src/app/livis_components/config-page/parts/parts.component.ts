...
export class PartsComponent implements OnInit {
  ...
  filterValue: any;
  skip: number = 0;
  displayedColumns = ['sl', 'model_number', 'model_description' ,'created_by',
  'created_by_time',
  'modified_by',
  'modified_by_time', 'defects', 'features',  'operation'];
  dataSource: any;
  ...
  addNewPart(model) {
    ...
    if (!this.partsForm.invalid) {
      ...
      const payload = {
        ...model.value, features : this.Features, defects : this.Defects, current_user : this.userInfo
      }
  ...
  updatePartInfo(model) {
    ...
    // if(!this.partsFormEdit.invalid){
      ...
      model.value.current_user = this.userInfo
      console.log(model.value);
